package service_test

import (
	"MamangRust/paymentgatewaygrpc/internal/cache"
	card_cache "MamangRust/paymentgatewaygrpc/internal/cache/card"
	"MamangRust/paymentgatewaygrpc/internal/domain/requests"
	"MamangRust/paymentgatewaygrpc/internal/repository"
	"MamangRust/paymentgatewaygrpc/internal/service"
	db "MamangRust/paymentgatewaygrpc/pkg/database/schema"
	"MamangRust/paymentgatewaygrpc/pkg/logger"
	"MamangRust/paymentgatewaygrpc/pkg/observability"
	"MamangRust/paymentgatewaygrpc/tests"
	"context"
	"testing"
	"time"

	"github.com/jackc/pgx/v5/pgxpool"
	"github.com/redis/go-redis/v9"
	"github.com/stretchr/testify/suite"
	sdklog "go.opentelemetry.io/otel/sdk/log"
)

type CardServiceTestSuite struct {
	suite.Suite
	ts          *tests.TestSuite
	dbPool      *pgxpool.Pool
	redisClient *redis.Client
	cardService service.CardService
	userRepo    repository.UserRepository
	cardRepo    repository.CardRepository
}

func (s *CardServiceTestSuite) SetupSuite() {
	ts, err := tests.SetupTestSuite()
	s.Require().NoError(err)
	s.ts = ts

	pool, err := pgxpool.New(s.ts.Ctx, s.ts.DBURL)
	s.Require().NoError(err)
	s.dbPool = pool

	opts, err := redis.ParseURL(s.ts.RedisURL)
	s.Require().NoError(err)
	s.redisClient = redis.NewClient(opts)

	queries := db.New(pool)
	repos := repository.NewRepositories(queries)
	s.userRepo = repos.User
	s.cardRepo = repos.Card

	logger.ResetInstance()
	lp := sdklog.NewLoggerProvider()
	log, _ := logger.NewLogger("test", lp)
	obs, _ := observability.NewObservability("test", log)
	cacheStore := cache.NewCacheStore(s.redisClient, log, nil)
	cacheCard := card_cache.NewCardMencache(cacheStore)

	s.cardService = service.NewCardService(service.CardServiceDeps{
		CardRepo:      repos.Card,
		UserRepo:      repos.User,
		Logger:        log,
		Observability: obs,
		Cache:         cacheCard,
	})
}

func (s *CardServiceTestSuite) TearDownSuite() {
	s.redisClient.Close()
	s.dbPool.Close()
	s.ts.Teardown()
}

func (s *CardServiceTestSuite) TestCardLifecycle() {
	ctx := context.Background()

	// Seed User
	user, err := s.userRepo.CreateUser(ctx, &requests.CreateUserRequest{
		FirstName: "Card",
		LastName:  "Owner",
		Email:     "card.service.owner@example.com",
		Password:  "password123",
	})
	s.Require().NoError(err)

	// 1. Create Card
	req := &requests.CreateCardRequest{
		UserID:       int(user.UserID),
		CardType:     "debit",
		ExpireDate:   time.Now().AddDate(2, 0, 0),
		CVV:          "123",
		CardProvider: "visa",
	}
	card, err := s.cardService.CreateCard(ctx, req)
	s.Require().NoError(err)

	// 2. Find By ID
	found, err := s.cardService.FindById(ctx, int(card.CardID))
	s.NoError(err)
	s.NotNil(found)
	s.Equal(card.CardNumber, found.CardNumber)

	// 3. Find By User ID
	foundByUser, err := s.cardService.FindByUserID(ctx, int(user.UserID))
	s.NoError(err)
	s.NotNil(foundByUser)
	s.Equal(card.CardNumber, foundByUser.CardNumber)
}

func TestCardServiceSuite(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test")
	}
	suite.Run(t, new(CardServiceTestSuite))
}
